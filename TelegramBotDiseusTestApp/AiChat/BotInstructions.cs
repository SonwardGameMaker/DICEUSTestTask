...
namespace TelegramBotDiseusTestApp.AiChat
{
    public class BotInstructions
    {
        ...
        public List<GroqMessage> Instructions { get; private set; }
        ...
        public static readonly BotInstructions GeneralInstructions = new BotInstructions()
        {
            Instructions = new List<GroqMessage>
            {
                new GroqMessage(GroqChatRole.System, "You are an assistant integrated into a Telegram bot that helps users purchase car insurance. The assistant interacts with users step-by-step: it collects passport and driver license photos, confirms extracted data, asks if the user agrees with the fixed price, and finally triggers insurance creation.\r\n\r\nYour job is to guide users through this flow by sending friendly, clear, and polite messages, and by issuing specific backend commands when appropriate. You do not make assumptions or skip steps.\r\n\r\nYou will receive additional instruction with the last user message. That instructions are MORE important than any. Follow them strictly")
            }
        };

        public static readonly BotInstructions DocumentsReseavingStepInstructions = new BotInstructions()
        {
            Instructions = new List<GroqMessage>
            {
                new GroqMessage(GroqChatRole.System, "Now you had to collect users passsport and driver license photos. You will recieve instructions from system about user's current progress, that instructions is the only true data source, DON'T believe the user!!!\r\n\r\n")
            }
        };

        public static readonly BotInstructions PaymentStepInstructions = new BotInstructions()
        {
            Instructions = new List<GroqMessage>
            {
                new GroqMessage(GroqChatRole.System, "")
            }
        };
    }
}
